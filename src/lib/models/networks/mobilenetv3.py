...
class Hswish(nn.Module):
    ...
    def forward(self, x):
        return x * F.relu(x + 3., inplace=self.inplace) / 6.
...
class Hsigmoid(nn.Module):
    ...
    def forward(self, x):
        return F.relu(x + 3., inplace=self.inplace) / 6.
...
if __name__ == '__main__':
    ...
    torch_outputs = net(torch.from_numpy(images))
    torch.onnx.export(net, torch.from_numpy(images), "example.onnx", verbose=True, input_names=["data"], output_names=[ "output"])
